# Script:
#   PR_U_EnablePrtScrSnippingTool_Det.ps1
# Run as:
#   User
# Author:
...
#   2022-11-01
# Description:
#   Checks if the printscreen button on the users device is set to open the snipping tool.
...
# Script
    ...
    if (Compare-RegKey -Path $RegPath -Key $RegKey -ExpectedValue $RegKeyValue) {
        Write-Log -Message "No further action needed."
    } else {
        Write-Log -Message "Triggering the Remediation."
        $ExitCode = 1
    }
