...
# Script
    ...
    if ($w32TimeStartupType -ne "Manual") {
        try {
            Write-Log "Changing the Startup Type to ""Manaul"""
            Set-Service -Name w32time -StartupType Manual
        } catch {
            Write-Log -Level Error -Message "Something went wrong when changing the Statup Type to ""Manual"""
            $ExitCode = 1
        }
    }
    ...
    if ($w32TimeStatus -ne "Running") {
        try {
            Write-Log "Starting the Windows Time Service"
            Start-Service W32Time
        } catch {
            Write-Log -Level Error -Message "Something went wrong when trying to start the Windows Time Service"
            $ExitCode = 1
        }
    }
    ...
    if ($STA -ne $RegKeyValue) {
        try {
            Write-Log "Setting this option to $RegKeyValue"
            New-ItemProperty -Path $RegPath -Name $RegKey -Value $RegKeyValue -PropertyType String -Force | Out-Null
        } catch {
            Write-Log -Level Error -Message "Something went wrong when changing this option to $RegKeyValue"
            $ExitCode = 1
        }
    }
    ...
    if ($STZA -eq 3) {
        ...
        try {
            Write-Log "Setting this option to ""Enabled"" (3)"
            New-ItemProperty -Path $RegPath -Name $RegKey -Value $RegKeyValue -PropertyType String -Force | Out-Null
        } catch {
            Write-Log -Level Error -Message "Something went wrong when changing this option"
            $ExitCode = 1
        }
    }
    ...
    # Change the current DateTime with 2 days.
    # This ensures the system picks up the difference in DateTime between the computer and NTP server.
