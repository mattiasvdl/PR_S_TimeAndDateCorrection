...
#   {author}
# Purpose:
#   Detection script to determine if {appName} is installed correctly

# Variables

    $appScriptURL = "$($env:PROGRAMDATA)\MattiasVdl\Scripts\{appScript}"
    $appIconURL = "$($env:PROGRAMDATA)\MattiasVdl\Icons\{appIcon}"
    $StartMenuShortcutURL = "$($env:PROGRAMDATA)\Microsoft\Windows\Start Menu\Programs\MattiasVdl\{appName}.lnk"
